#CentOS7安装Openbox桌面+配置vncserver+Fcitx输入法+搜狗输入
#lxyoucan@163.com
#2021年7月30日
...
#选择一个下载源
echo -n "${tty_green}
欢迎使用，CentOS7安装Openbox+Vnc小助手！本脚本做以下的事情：
- 安装tigervnc-server
- 配置vncserver
- 安装Openbox桌面
- 开放防火墙VNC服务端口
...
手动安装教程：https://blog.csdn.net/lxyoucan/article/details/119081236
...
"1")
    ...
    echo "${tty_green}正在创建用户${MY_VNC_USER},如果用户已经存在也不影响${tty_reset}"
    adduser "${MY_VNC_USER}"
    ...
    # 安装OpenBox相关依赖
    echo "${tty_green}正在安装OpenBox相关依赖${tty_reset}"
    yum -y install openbox xfce4-terminal tint2
    ...
    echo "${tty_green}正在配置/etc/xdg/openbox/autostart${tty_reset}"

    #为了支持多次运行，在此清空配置文件内容
    echo 'tint2 &' > /etc/xdg/openbox/autostart
    #网络组件在部分服务器上显示不准确，明明是可以上网，显示网络示未连接，暂时不加
    ...
    echo "openbox-session" >> "/home/${MY_VNC_USER}/.vnc/xstartup"
    echo "vncserver -kill $DISPLAY" >> "/home/${MY_VNC_USER}/.vnc/xstartup"
    #授权
    #chmod +x "/home/${MY_VNC_USER}/.vnc/xstartup"
    chmod 755 "/home/${MY_VNC_USER}/.vnc/xstartup"
    chown -R  "${MY_VNC_USER}" "/home/${MY_VNC_USER}/.vnc/"

    #openbox中搜狗输入第一次启动服务会导致输入法异常，所以要重新一下服务
    #不清楚为什么 这个开启与stop为什么特别的慢
    #systemctl start "vncserver@:${MY_VNC_PORT}"
    #systemctl stop "vncserver@:${MY_VNC_PORT}"
    #安装搜狗输入法
